    ...
    <div className="min-h-screen bg-[#F4F7F9] p-8 font-sans">
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg flex flex-col max-h-[90vh] overflow-hidden">
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 bg-white flex-shrink-0">
              <button
                type="button"
                onClick={resetForm}
                className="flex items-center gap-1.5 text-gray-400 hover:text-[#0B1F3A] font-bold text-xs uppercase tracking-widest transition-colors"
              >
                ← Back
              </button>
              <h2 className="text-base font-black text-[#0B1F3A] flex items-center gap-2">
                <span className="w-1.5 h-5 bg-[#0B1F3A] rounded-full"></span>
            ...
            <form onSubmit={handleSave} className="flex flex-col flex-1 overflow-hidden">
              <div className="overflow-y-auto flex-1 px-6 py-5 space-y-4">
                ...
                <div>
                  ...
                  <input className="w-full bg-gray-50 border-none p-3 rounded-xl focus:ring-2 focus:ring-[#0B1F3A] outline-none text-sm" value={fileName} onChange={e => setFileName(e.target.value)} placeholder="e.g. BCA/2026/042" required />
                </div>
                ...
                <div>
                  ...
                  <div className="relative" data-matter-search>
                    <input
                      ...
                      className="w-full bg-gray-50 border-none p-3 rounded-xl focus:ring-2 focus:ring-[#0B1F3A] outline-none text-sm"
                      ...
                      <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-gray-200 rounded-xl shadow-lg z-20 max-h-44 overflow-y-auto">
                        {filteredMatters.length > 0 ? filteredMatters.map((f, i) => (
                          <button key={i} type="button"
                            ...
                            className="w-full text-left px-4 py-2.5 hover:bg-blue-50 border-b border-gray-50 last:border-0 text-sm font-semibold text-[#0B1F3A]"
                          >{f.label}</button>
                ...
                <div>
                  ...
                  <textarea
                    rows={2}
                    className="w-full bg-gray-50 border-none p-3 rounded-xl focus:ring-2 focus:ring-[#0B1F3A] outline-none resize-none text-sm font-medium text-gray-700"
                ...
                <div>
                  ...
                  <label className={`flex items-center justify-between w-full p-3 rounded-xl border-2 border-dashed cursor-pointer transition-all ${scannedFile ? 'border-blue-500 bg-blue-50' : 'border-gray-200 bg-gray-50 hover:border-blue-300'}`}>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <div className="p-6 border-b bg-white">
          <h3 className="text-lg font-bold text-[#0B1F3A]">Accounts Receivable</h3>
        </div>
                        ...
                        <div className="flex flex-wrap gap-1 justify-center">
                          ...
                          {inv.scannedInvoiceUrl && (
                            <a href={inv.scannedInvoiceUrl} target="_blank" rel="noreferrer"
                              className="underline hover:bg-blue-100 text-blue-600 text-[10px] font-black px-2.5 py-1.5 rounded-lg transition-colors">View</a>
                          )}
                          <button
                            onClick={() => handleEdit(inv)}
                            className="underline  hover:bg-slate-200 text-slate-600 text-[10px] font-black px-2.5 py-1.5 rounded-lg transition-colors"
                          >Edit</button>
                          <button
